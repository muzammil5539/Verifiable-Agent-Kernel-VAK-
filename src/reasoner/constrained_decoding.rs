...
impl GrammarRule {
    ...
    pub fn json_schema(name: impl Into<String>, schema: serde_json::Value) -> Self {
        Self {
            name: name.into(),
            rule_type: GrammarType::JsonSchema,
            definition: schema,
            required: true,
            error_message: None,
        }
    ...
    pub fn regex(name: impl Into<String>, pattern: impl Into<String>) -> Self {
        Self {
            name: name.into(),
            rule_type: GrammarType::Regex,
            definition: serde_json::json!({ "pattern": pattern.into() }),
            required: true,
            error_message: None,
        }
    }
    ...
    pub fn enum_values(name: impl Into<String>, values: Vec<String>) -> Self {
        Self {
            name: name.into(),
            rule_type: GrammarType::Enum,
            definition: serde_json::json!({ "values": values }),
            required: true,
            error_message: None,
        }
    }
...
impl ConstrainedDecoder {
            ...
            if matches!(rule.rule_type, GrammarType::Regex) {
                if let Some(pattern) = rule.definition.get("pattern").and_then(|p| p.as_str()) {
                    let regex = Regex::new(pattern).map_err(|e| {
                        ConstraintError::InvalidConfig(format!("Invalid regex: {}", e))
                    })?;
    ...
    pub fn validate(&self, output: &str) -> ValidationResult {
        ...
        if !valid && self.grammar.attempt_repair {
            result.suggestions = self.generate_suggestions(output);
        }
        result
    }
    ...
    fn validate_rule(
        ...
        rule: &GrammarRule,
        output: &str,
    ) -> ConstraintResult<Option<serde_json::Value>> {
    ...
    fn validate_json_schema(
        ...
        let value: serde_json::Value = serde_json::from_str(output)
            .map_err(|e| ConstraintError::SchemaValidation(format!("Invalid JSON: {}", e)))?;
        ...
        if let Some(expected_type) = schema.get("type").and_then(|t| t.as_str()) {
            ...
            if actual_type != expected_type {
                return Err(ConstraintError::SchemaValidation(format!(
                    "Expected '{}', got '{}'",
    ...
    fn validate_datalog(&self, output: &str) -> ConstraintResult<Option<serde_json::Value>> {
        ...
        for line in lines {
            if fact_pattern.is_match(line) || rule_pattern.is_match(line) {
                ...
                return Err(ConstraintError::DatalogSyntax(format!(
                    "Invalid Datalog: {}",
                    line
                )));
            }
        ...
        if facts.is_empty() {
            return Err(ConstraintError::DatalogSyntax(
                "No valid Datalog facts".to_string(),
            ));
        }
    ...
    fn validate_regex(
        ...
        let regex = self
            .compiled_patterns
            .get(&rule.name)
            .ok_or_else(|| ConstraintError::InvalidConfig("Pattern not compiled".to_string()))?;
        if regex.is_match(output) {
            Ok(Some(serde_json::Value::String(output.to_string())))
        } else {
            Err(ConstraintError::PatternMismatch(format!(
                "Doesn't match pattern for '{}'",
                rule.name
            )))
        }
    }
    ...
    fn validate_enum(
        ...
        let values = rule
            ...
            .ok_or_else(|| {
                ConstraintError::InvalidConfig("Enum values not specified".to_string())
            })?;
        let trimmed = output.trim();
        for value in values {
            if value.as_str() == Some(trimmed) {
                return Ok(Some(serde_json::Value::String(trimmed.to_string())));
            }
        }
        Err(ConstraintError::GrammarViolation(format!(
            "'{}' not in enum",
            trimmed
        )))
    }
    ...
    fn generate_suggestions(&self, output: &str) -> Vec<RepairSuggestion> {
            ...
            && serde_json::from_str::<serde_json::Value>(trimmed).is_err()
        {
            suggestions.push(RepairSuggestion {
                repair_type: RepairType::FixSyntax,
                description: "JSON has syntax errors".to_string(),
                suggested_fix: Some("Check quotes, commas, brackets".to_string()),
                confidence: 0.8,
            });
        }
            ...
            if let Some(end) = trimmed.rfind('}') {
                if end > start {
                    ...
                    if serde_json::from_str::<serde_json::Value>(potential).is_ok() {
                        suggestions.push(RepairSuggestion {
                            repair_type: RepairType::ExtractSubstring,
                            description: "JSON embedded in text".to_string(),
                            suggested_fix: Some(format!("Extract: {}", potential)),
                            confidence: 0.9,
                        });
                    }
                }
...
mod tests {
    ...
    fn test_json_validation() {
        let grammar = OutputGrammar::json_schema(
            "test",
    ...
    fn test_enum_validation() {
        let grammar = OutputGrammar::new("test").with_rule(GrammarRule::enum_values(
            "status",
