...
impl MemoryItem {
    ...
    pub fn tool_call(tool_name: impl Into<String>, content: impl Into<String>) -> Self {
        Self::new(ItemType::ToolCall { tool_name: tool_name.into() }, content)
    }
    
    /// Create a tool result item
    pub fn tool_result(tool_name: impl Into<String>, content: impl Into<String>) -> Self {
        Self::new(ItemType::ToolResult { tool_name: tool_name.into() }, content)
    }
    ...
    pub fn to_message(&self) -> Message {
        match &self.item_type {
            ...
            ItemType::Observation => {
                Message::user(format!("[Observation] {}", self.content))
            }
            ItemType::Thought => {
                Message::assistant(format!("[Thought] {}", self.content))
            }
            ItemType::Summary { .. } => {
    ...
    pub async fn summarize(&mut self) -> Result<(), WorkingMemoryError> {
        ...
        let content_to_summarize = items_to_summarize
            ...
            .map(|item| {
                ...
                format!("{}{}: {}", timestamp, format_item_type(&item.item_type), item.content)
            })
            .collect::<Vec<_>>()
            .join("\n");
        ...
        let items_to_remove: std::collections::HashSet<_> = items_to_summarize.iter().map(|i| i.id).collect();
...
Be concise but complete. Focus on information that would be needed to continue the conversation meaningfully."#;
    ...
    pub fn clear_except_system(&mut self) {
        let system_items: VecDeque<_> = self.items
            .drain(..)
...
mod tests {
    ...
    fn test_summary_item() {
        ...
        assert!(matches!(summary.item_type, ItemType::Summary { items_summarized: 5, .. }));
    ...
    async fn test_summarize_with_mock_llm() {
        let llm = Arc::new(MockLlmProvider::always("This is a summary of the conversation."));
        let config = WorkingMemoryConfig::default()
            .with_max_tokens(50)
        ...
        let has_summary = items.iter().any(|item| matches!(item.item_type, ItemType::Summary { .. }));
        assert!(has_summary);
    }
